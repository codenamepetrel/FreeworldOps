...
python3 "$SCRIPT_DIR/oscp_db_setup.py"

# Add alias to zshrc
ALIAS_LINE="alias oscp='python3 $SCRIPT_DIR/oscp.py'"

if grep -q "alias oscp=" ~/.zshrc 2>/dev/null; then
    sed -i "s|alias oscp=.*|$ALIAS_LINE|" ~/.zshrc
    echo -e "${GREEN}[+] Updated existing oscp alias in ~/.zshrc${RESET}"
else
    echo "$ALIAS_LINE" >> ~/.zshrc
    echo -e "${GREEN}[+] Added oscp alias to ~/.zshrc${RESET}"
fi
...
echo ""
echo -e "${GREEN}${BOLD}[+] All done! Run: source ~/.zshrc then type oscp${RESET}"
